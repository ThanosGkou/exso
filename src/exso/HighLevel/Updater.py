...
class Updater:
    ...
    def single(self, report_name, use_lake_version, retroactive_update, keep_raw = False, refresh_database = False):
        ...
        if self.refresh_requirements[report_name]:
            ...
            print('\tIt seems like you upgraded to the latest exso version, which brought some changes to the specific report ({}). This report\'s data(base), just for this time, will be fully refreshed instead of just updated.'.format(report_name))
...
class LogSplitter:
    ...
    def extract(self, report_name):
        ...
        if _warnings:
            print('\t\t*There were some warnings activated during the update process. \n\t\t Chances are that they are completely harmless, and not of any interest for a user, but if you were expecting something specific, have a look at the log files, at the very bottom. (C:/Users/yourusername/AppData/Local/Temp/exso/logs/latest_logs/{}.log)\n\n'.format(report_name))
